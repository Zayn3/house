...
<script type="text/javascript">
    $(function () {
        $("#name").blur(function () {
            $.post("checkName",{"username":$("#name").val()},
                function (data) {
                    if (data.result==0){
                        $("#spa").html("用户名可用").css("color","green");
                    }else {
                        $("#spa").html("用户名已被注册").css("color","red");
                    }
                },"json")
        })
    })
...
<TABLE class=field>
  ...
  <TR>
    ...
    <TD><INPUT class=text type=text name=name id="name"><span id="spa"></span></TD></TR>
  <TR>
